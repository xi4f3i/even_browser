...
impl URL {
    ...
    pub fn request(&self) -> String {
        ...
        if self.scheme == "https" {
            ...
            self.send_request(tls_stream)
        } else {
            self.send_request(stream)
        }
    }

    fn send_request<S: Read + Write>(&self, mut stream: S) -> String {
        let request = format!(
            "GET {} HTTP/1.0\r\n\
             Host: {}\r\n\
             User-Agent: Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36 EvenBrowser/0.1\r\n\
    ...
    fn parse_response<T: Read>(&self, reader: &mut BufReader<T>) -> String {
        ...
        if headers.contains_key("transfer-encoding") {
            eprintln!("Warning: transfer-encoding not supported yet");
        }
        if headers.contains_key("content-encoding") {
            eprintln!("Warning: content-encoding not supported yet");
        }

        let mut content = match content_length {
    ...
    pub fn resolve(&self, url_str: &str) -> URL {
        ...
        URL::new(&format!(
            "{}://{}:{}{}",
            self.scheme, self.host, self.port, url
        ))
    }
}
