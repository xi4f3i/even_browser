...
mod tests {
    ...
    fn test_implicit_elements() {
        ...
        assert_eq!(document.child_nodes().len(), 1, "Document should have 1 child (html)");
        let html = &document.child_nodes()[0];
        assert_eq!(html.get_element().unwrap().tag_name(), "html");

        let html_children = html.child_nodes();
        let head = html_children.iter().find(|n| matches!(n.get_element(), Some(e) if e.tag_name() == "head")).expect("Missing head");
        let body = html_children.iter().find(|n| matches!(n.get_element(), Some(e) if e.tag_name() == "body")).expect("Missing body");

        assert_eq!(head.get_element().unwrap().tag_name(), "head");
        assert_eq!(body.get_element().unwrap().tag_name(), "body");
    }

    #[test]
    fn test_minimal_html() {
        let parser = HtmlParser::new("<html><head></head><body></body></html>");
        let document = parser.parse();
        print_node_tree(&document, 0);

        let html = document.child_nodes().iter().find(|n| matches!(n.get_element(), Some(e) if e.tag_name() == "html")).expect("Missing html");
        
        let html_children = html.child_nodes();
        assert!(html_children.iter().any(|n| matches!(n.get_element(), Some(e) if e.tag_name() == "head")));
        assert!(html_children.iter().any(|n| matches!(n.get_element(), Some(e) if e.tag_name() == "body")));
    }

    #[test]
    fn test_nested_elements() {
        let parser = HtmlParser::new("<div><p>text</p></div>");
        let document = parser.parse();
        print_node_tree(&document, 0);

        let html = document.child_nodes().iter().find(|n| matches!(n.get_element(), Some(e) if e.tag_name() == "html")).expect("Missing html");
        let body = html.child_nodes().iter().find(|n| matches!(n.get_element(), Some(e) if e.tag_name() == "body")).expect("Missing body");

        assert_eq!(body.child_nodes().len(), 1);
        let div = &body.child_nodes()[0];
        assert_eq!(div.get_element().expect("div should be an element").tag_name(), "div");

        assert_eq!(div.child_nodes().len(), 1);
        let p = &div.child_nodes()[0];
        assert_eq!(p.get_element().expect("p should be an element").tag_name(), "p");

        assert_eq!(p.child_nodes().len(), 1);
        let text = &p.child_nodes()[0];
        assert_eq!(text.get_text().expect("text should be a text node").data(), "text");
    }

    #[test]
    fn test_character_insertion_merging() {
        let parser = HtmlParser::new("a b");
        let document = parser.parse();
        print_node_tree(&document, 0);
        ...
        let text = body.child_nodes().iter().find(|n| n.get_text().is_some()).expect("Missing text node");
        assert_eq!(text.get_text().unwrap().data(), "a b");
    }

    #[test]
    fn test_comments() {
        let parser = HtmlParser::new("<!-- doc comment --><html><!-- head comment --><head></head><body></body></html><!-- after comment -->");
        let document = parser.parse();
        print_node_tree(&document, 0);

        let doc_children = document.child_nodes();
        assert!(doc_children.iter().any(|n| match n.subtype() {
             NodeSubtype::Comment(c) => c.data().contains("doc comment"),
             _ => false,
        }), "Missing doc comment");

        let html = doc_children.iter().find(|n| matches!(n.get_element(), Some(e) if e.tag_name() == "html")).expect("Missing html");
        
        assert!(html.child_nodes().iter().any(|n| match n.subtype() {
            NodeSubtype::Comment(c) => c.data().contains("head comment"),
            _ => false,
        }), "Missing head comment");
    }
}
